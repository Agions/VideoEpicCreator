...
"""
    ...
    def run_performance_test():
        ...
        start_time = time.time()
        for i in range(100):
            config_manager.save_plugin_config(f"plugin_{i}", {"value": i})
            config_manager.load_plugin_config(f"plugin_{i}")
    ...
    def run_comprehensive_test():
        ...
        try:
            ...
            if len(search_results) == 5:
                ...
                print(f"   ✗ 插件搜索功能异常: 找到 {len(search_results)} 个结果")
                success = False

            # 测试类型筛选
            effect_plugins = marketplace.get_plugins_by_type(PluginType.EFFECT)
            if len(effect_plugins) == 5:
                print("   ✓ 插件类型筛选正常")
            else:
                print(f"   ✗ 插件类型筛选异常: 找到 {len(effect_plugins)} 个效果插件")
                success = False
        ...
        try:
            # 测试不同风险等级的代码
            test_cases = [
                ("安全代码", "def safe_func(): return 1", "LOW"),
                ("警告代码", "import subprocess", "MEDIUM"),
                ("危险代码", "import os; os.system('rm -rf /')", "HIGH")
            ]
        ...
        try:
            ...
            config_manager.save_plugin_config("complex_plugin", complex_config)
            loaded_config = config_manager.load_plugin_config("complex_plugin")
